...
const co2savingsMultiplier = 0.13 // EU limit for new cars = 0.13 kg/km
...
class Loyalty {
  static loyaltyPoints (route) {
    ...
    if (route.vanETAatStartVBS && route.toStartRoute.routes[0].legs[0].duration) {
      ...
      waitingTime = Math.round((((route.vanETAatStartVBS - new Date()) % 86400000) % 3600000) / 60000) - route.toStartRoute.routes[0].legs[0].duration.value
      if (waitingTime > 10) {
    ...
    co2savings = vanDistance * co2savingsMultiplier
